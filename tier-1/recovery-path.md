...
1. **cloud-storage version history** (default for `data/` cloud-storage-linked projects): right-click → Version history (single file) or Rewind folder (folder-level). 30+ days of history per file.
2. **local-backup / local backup snapshot** (default for non-cloud-storage content).
...
**Tell the user about the recovery path BEFORE proposing destructive operations.** If the only recovery path is "re-run the pipeline (4 hours)", the cost calculus is different from "cloud-storage restore (30 seconds)".
