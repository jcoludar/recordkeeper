...
> `python <recordkeeper>/tools/assemble.py <recordkeeper> <this-repo>`
"""
...
def deploy_hooks(
    *,
    ...
    for src in src_paths:
        dst = dst_dir / src.name
        shutil.copy2(src, dst)
        # Hooks are run by the Claude Code harness; guarantee they're executable
        # regardless of the source's committed mode (git / cross-platform checkouts
        # can drop the +x bit, and shutil.copy2 preserves whatever it finds). The
        # settings fragments invoke hooks via `/usr/bin/env python3`, but keeping
        # +x means direct invocation works too.
